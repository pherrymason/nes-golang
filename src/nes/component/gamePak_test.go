...
func Test_read_gamePak_without_trainer(t *testing.T) {
	...
	value := gamePak.read(GAMEPAK_ROM_START)
...
func Test_write(t *testing.T) {
	gamePak := aSampleGamePak(false)
	gamePak.write(GAMEPAK_ROM_START+1, 0x02)
...
func Test_write_on_mirror_space(t *testing.T) {
	...
	assert.Equal(t, byte(0x02), gamePak.prgROM[0xc100-0x4000-GAMEPAK_ROM_START])
}
...
func aSampleGamePak(withTrainer bool) GamePak {
	...
	data[0xC100-0x4000-GAMEPAK_ROM_START] = 0xFF
