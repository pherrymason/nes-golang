...
func myHex(n uint32, d int) string {
...
func (cpu *Cpu6502) Disassemble(start Address, end Address) map[Address]string {
	...
	for addr <= uint32(end) {
		...
		if len(instruction.Name()) == 0 {
			sInst += "0x" + myHex(uint32(opcode), 2) + "? "
		} else {
